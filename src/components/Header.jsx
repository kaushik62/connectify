...
function Header() {
  ...
  return (
    <header className="py-3 flex items-center justify-between bg-white shadow-md fixed top-0 z-50 w-full px-5">
      {/* Logo - Always visible */}
      <Link
        to="/"
        className="text-3xl font-bold bg-gradient-to-r from-fuchsia-600 via-pink-500 to-rose-500 text-transparent bg-clip-text pl-12"
      >
        Connectify
      </Link>

      {/* Desktop Nav - Visible only when logged in */}
      {isLoggedIn && (
      ...
      {/* Logout Button (Desktop) - Visible only when logged in */}
      {isLoggedIn && (
        <button
      ...
      {/* Hamburger Icon (Mobile) - Show only when logged in */}
      {isLoggedIn && (
        <button
          onClick={toggleMenu}
      ...
      {/* Mobile Menu - Show only when logged in and menu is open */}
      {isLoggedIn && menuOpen && (
